...
for ind, entity in enumerate(entity_sixGroup):
    ...
    if entity['cnt']>0:
        data['nodes'].append({'pmid':pmid_list[ind],
...
pmid2dataNodesInd={} # when get link from entity to gbc, need this

for ind, node in enumerate(data['nodes']):
    pmid2dataNodesInd.update({node['pmid']:ind})

offset = len(data['nodes'])
for pmid in topxgbc_pmids:
    pmid2dataNodesInd.update({pmid: offset})
    offset+=1
    data['nodes'].append({'pmid':pmid,
                          'title':gbc_titles[gbc_pmid2ind[pmid]],
                          'size':topxgbc_pmid2cnt[pmid],
                          'group':2})
...
for ind, node in enumerate(data['nodes']):            
    if 'sixGroup' in node:
        ...
        for i in range(1,7):
            for pmid in sixG[i]:
                if pmid in topxgbc_pmids_set:
                    nodeInd = pmid2dataNodesInd[pmid]
                    data['links'].append({'source':ind, 'target':nodeInd, 'value':i-1})


import pickle
with open(f'depth{depth_constrain}_nodes_visitedConstrain_{top_x}_graph_data', 'wb') as f:
    print(f'depth{depth_constrain}_nodes_visitedConstrain_{top_x}_graph_data')
    pickle.dump(data, f)
